use crate::GaussianSplattingScene;
...
impl GaussianSplattingScene {
    ...
    pub(crate) fn parse_file_header(
        ...
        loop {
            ...
            if line.starts_with("element vertex ") {
                self.splat_count = line[15..line.len() - 1].parse().unwrap();
            }
            line.clear();
        ...
        if !success {
            let error = "GaussianSplattingScene::parse_file_header(): ERROR: the file is not correctly formatted.";
            tracing::info!("{}, i={}", error, i);
            return Err(error.to_string());
        }

        self.file_header_size = reader.stream_position().unwrap() as u16;
        let cursor = reader.into_inner();
        tracing::info!(
            "GaussianSplattingScene::parse_file_header(): i={}, file_header_size={}, splat_count={}",
            i,
    ...
    /// Loads an entire PLY file into memory
    pub(crate) fn load(&mut self, cursor: &mut Cursor<Vec<u8>>) {
        let mut serialized_splats =
            vec![SerializedSplat::default(); self.splat_count];
        cursor.seek(SeekFrom::Start(self.file_header_size as u64)).unwrap();
        cursor
        ...
        tracing::info!(
            "GaussianSplattingScene::load(): size_list[0]={}, size_list[-1]={}",
        ...
        for i in 0..self.splat_count {
            ...
            let mut end = start + 3 * 4;
            {
                ...
                let position: &mut [f32] =
                    transmute_slice_mut::<_, f32>(&mut buffer[start..end]);
            ...
            end = start + 4;
            {
                // read 4x u8
                let rgba: &mut [u8] =
                    transmute_slice_mut::<_, u8>(&mut buffer[start..end]);
    ...
    pub fn generate_texture(&mut self) {
        ...
        let f_buffer: &[f32] =
            transmute_slice::<_, f32>(self.buffer.as_slice());
        let u_buffer: &[u8] = transmute_slice::<_, u8>(self.buffer.as_slice());

        let texwidth = 1024 * 2 as usize;
        let texheight =
            ((2 * self.splat_count) as f64 / texwidth as f64).ceil() as usize;
        let len_texdata = texwidth * texheight * 4 as usize; // 4 components per pixel (RGBA)
        tracing::info!("GaussianSplattingScene::generate_texture(): texheight={}, len_texdata={}", texheight, len_texdata);
        let mut texdata = vec![0_u32; len_texdata];

        {
            let texdata_f =
                transmute_slice_mut::<_, f32>(texdata.as_mut_slice());
            for i in 0..self.splat_count {
        ...
        {
            let texdata_c =
                transmute_slice_mut::<_, u8>(texdata.as_mut_slice());
            for i in 0..self.splat_count {
        ...
        for i in 0..self.splat_count {
            ...
            texdata[index_f + 4] =
                pack_half_2x16(4.0 * sigma[0], 4.0 * sigma[1]); // a, b
            texdata[index_f + 5] =
                pack_half_2x16(4.0 * sigma[2], 4.0 * sigma[3]); // c, d
            texdata[index_f + 6] =
                pack_half_2x16(4.0 * sigma[4], 4.0 * sigma[5]); // e, f
        }
    ...
    pub fn sort(
        scene: &Arc<Self>, view_proj: &[f32], bus: &mut Bus<Vec<u32>>,
        n_threads: usize,
    ) {
        ...
        let f_buffer: &[f32] =
            transmute_slice::<_, f32>(scene.buffer.as_slice());
        ...
        */
        ...
        //tracing::info!("GaussianSplattingScene::sort(): max_depth={:?}, min_depth={:?}", max_depth, min_depth);
        ...
        for i in 0..scene.splat_count {
            let depth =
                ((size_list[i] - min_depth) as f32 * depth_inv).floor() as i32;
    ...
    /// Sorts the splats based on their depth using 16-bit single-pass counting sort
    pub fn sort2(
        scene: &Self, view_proj: &[f32], bus: &mut Bus<Vec<u32>>,
        n_threads: usize,
    ) {
        if scene.buffer.is_empty() {
            return;
...
impl Default for SerializedSplat {
    fn default() -> Self {
        unsafe {
            std::mem::MaybeUninit::<SerializedSplat>::zeroed().assume_init()
        }
    }
}
...
pub fn pack_half_2x16(x: f32, y: f32) -> u32 {
    ...
    let result =
        u32::from(x_half.to_bits()) | (u32::from(y_half.to_bits()) << 16);
    result // & 0xFFFFFFFF
}
